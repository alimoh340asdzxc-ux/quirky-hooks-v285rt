...
function App() {
  const [products, setProducts] = useState([]); // هتبدأ بجدول فاضي لحد ما تحمل
  ...
  // جلب البيانات الحقيقية عند فتح الموقع
  useEffect(() => {
    fetch("https://fakestoreapi.com/products")
      .then((res) => res.json())
      .then((data) => {
        // تحويل البيانات عشان تليق مع ستايل LUXE وكودك القديم
        const formattedData = data.map((item) => ({
          id: item.id,
          name: item.title,
          price: Math.round(item.price * 50), // تحويل السعر لجنيه مصري تقريباً
          image: item.image,
          category:
          ...
          stock: Math.floor(Math.random() * 20) + 1, // رقم عشوائي للمخزون
          sizes: ["S", "M", "L", "XL"],
        }));
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
        <h1 className="text-3xl font-serif animate-pulse">
          LUXE | Loading Collection...
        </h1>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
            ...
            >
              <div className="relative h-80 overflow-hidden bg-gradient-to-br from-gray-50">
                <img
                  src={product.image}
                  alt={product.name}
                  className="w-full h-full object-contain p-4 group-hover:scale-110 transition-all duration-700"
                />
              ...
              <div className="p-8">
                <h3 className="font-bold text-lg mb-4 h-14 overflow-hidden leading-tight group-hover:text-amber-700 transition-colors">
                  {product.name}
                </h3>
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-gray-900 to-black text-white py-4 px-8 rounded-2xl font-bold text-lg hover:from-amber-500 hover:to-yellow-500 shadow-xl transition-all duration-500"
                >
                  أضف للسلة
                </button>
              </div>
      ...
      {/* Cart Sidebar */}
      {cart.length > 0 && (
        <div className="fixed right-0 top-0 h-full w-96 bg-white shadow-3xl transform transition-transform duration-500 z-50">
          ...
          <div className="p-8 border-t">
            <button className="w-full bg-gradient-to-r from-emerald-500 to-green-600 text-white py-4 rounded-2xl font-bold">
              إتمام الشراء
            </button>
          </div>
        </div>
